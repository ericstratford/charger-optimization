...
m1 = folium.Map(location=[32.7157, -117.1611], zoom_start=12)

# Add public parking lots to the map
for _, row in parking_lots_gdf.iterrows():
    if row.geometry.geom_type == "Point":
        folium.CircleMarker(
            ...
            fill_color="blue",
            fill_opacity=0.6,
            popup=f"Type: {row['access']}"
        ).add_to(m1)
    elif row.geometry.geom_type in ["Polygon", "MultiPolygon"]:
        folium.GeoJson(
            row.geometry,
            style_function=lambda x: {"color": "blue", "fillColor": "blue", "weight": 1, "fillOpacity": 0.4},
            tooltip=f"Type: {row['access']}"
        ).add_to(m1)
print("All Lots Shape:", parking_lots_gdf.shape)
print("Public Lots Shape:", public_lots_gdf.shape)
print(parking_lots_gdf.columns)
m1
...
# Plot the Parking Lots over a Zoning Map
...
# Display the map
print("Map of City Zoning Overlaid by Parking Lots")
m2
...
# #### Plot Chargers and Parking Lots

# In[15]:


# Create a Folium map
m3 = folium.Map(location=[32.7157, -117.1611], zoom_start=12)
...
for _, row in cs_gdf.iterrows():
    if row.geometry.geom_type == "Point":
        folium.Marker(
            location=[row.geometry.y, row.geometry.x],
            popup=f"Charging Station ID: {row['id']}",  # Adjust attribute accordingly
            tooltip="Charging Station",
            icon=folium.Icon(color="blue", icon="info-sign")
        ).add_to(m3)

# Show the map
m3.save('parking_lots_and_chargers.html')
m3
...
# Import combined lot and charger saved dataset
...
# City Zoning - Empirical Zoning Statistical Probability Score
from sklearn.preprocessing import StandardScaler, MinMaxScaler
# Perform the spatial join to map chargers to zoning data
cs_zoned = gpd.sjoin(cs_gdf, zoning_data, how="left")

# Drop unnecessary columns from zoning_data
cs_zoned = cs_zoned.drop(columns=['index_right', 'imp_date', 'ordnum', 'objectid'], errors='ignore')

# Drop chargers with no assigned zone
cs_zoned['zone_type'] = cs_zoned['zone_type'].dropna()

# Compute charger density in each zone (probability-like score)
zone_scores = cs_zoned['zone_type'].value_counts(normalize=True).to_dict()

# Map the scores back to the dataframe
cs_zoned['std_zone_score'] = cs_zoned['zone_type'].map(zone_scores)

# Optionally, normalize the scores between 0 and 1
scaler = MinMaxScaler()
cs_zoned['std_zone_score'] = scaler.fit_transform(cs_zoned[['std_zone_score']])

# Check results
print(cs_zoned[['zone_type', 'std_zone_score']].drop_duplicates().sort_values(by='std_zone_score'))
...
cs_lots_scored = cs_lots_scored.merge(
    cs_zoned[['zone_type', 'std_zone_score']].drop_duplicates(),
    on='zone_type',
    how='left'
)

cs_lots_scored['std_zone_score'] = cs_lots_scored['std_zone_score'].fillna(cs_lots_scored['std_zone_score'].min())
...
# Assignment weights to each scoring type as through a customizable parameter
...
cs_lots_scored['traffic_score'] = cs_lots_scored['percentile_traffic'] * 0.4
cs_lots_scored['zone_score'] = cs_lots_scored['std_zone_score'] * 0.1
